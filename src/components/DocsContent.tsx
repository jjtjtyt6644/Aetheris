...
import { X, BookOpen, Users, Sparkles, Cloud, Shield, Zap, ChevronRight, Lock, Scale, Home } from "lucide-react";
...
export default function DocsContent({ onClose, isPage = false }: DocsContentProps) {
  const [activeTab, setActiveTab] = useState<"architecture" | "structure" | "security" | "moderation" | "legal">("architecture");

  const containerClasses = isPage
    ? "w-full h-screen flex flex-col md:flex-row bg-zinc-900/40 transition-all backdrop-blur-2xl"
    : "relative w-full max-w-5xl flex flex-col md:flex-row overflow-hidden bg-zinc-900/95 border border-white/10 shadow-[0_0_80px_rgba(0,0,0,0.6)] h-[85vh] rounded-3xl";

  return (
    <div className={containerClasses}>
      {/* Sidebar / Table of Contents */}
      <div className={`w-full md:w-[260px] flex-shrink-0 p-8 border-b md:border-b-0 md:border-r border-white/5 bg-white/[0.01] flex flex-col ${isPage ? "md:sticky md:top-0 md:h-screen" : "h-full"}`}>
        <div className="flex items-center gap-3 mb-10">
          <div className="p-2.5 rounded-xl bg-indigo-500/20 text-indigo-400">
            <BookOpen className="w-5 h-5" />
          </div>
          <div>
            <h2 className="font-bold text-base tracking-tight leading-none mb-1">Documentation</h2>
        ...
        <div className="mt-auto pt-8 flex flex-col gap-4">
          {isPage && (
            <Link
              href="/"
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-white/60 hover:text-white text-xs font-medium transition-all border border-white/5 mb-4"
            >
      ...
      {/* Content Area */}
      <div className={`flex-1 overflow-y-auto custom-scrollbar bg-black/20 ${isPage ? "h-full p-8 md:p-16 lg:p-24" : "p-12 h-full"}`}>
        {!isPage && onClose && (
        ...
        <div className="max-w-3xl mx-auto">
            ...
            <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="space-y-10">
              <section className="space-y-6">
                <div className="space-y-2">
                  ...
                  <h1 className="text-4xl md:text-5xl font-bold text-white tracking-tight">System Architecture</h1>
                </div>
          ...
          {activeTab === "structure" && (
            <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="space-y-10">
              <div className="space-y-2">
                <Tag text="Source Map" />
                <h1 className="text-4xl md:text-5xl font-bold text-white tracking-tight">Project Structure</h1>
              </div>
          ...
          {activeTab === "security" && (
            <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="space-y-10">
              <div className="space-y-2">
                ...
                <h1 className="text-4xl md:text-5xl font-bold text-white tracking-tight">Governance & AI</h1>
              </div>
          ...
          {activeTab === "moderation" && (
            <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="space-y-10">
              <div className="space-y-2">
                <Tag text="Operations" />
                <h1 className="text-4xl md:text-5xl font-bold text-white tracking-tight">Moderation Hub</h1>
              </div>
          ...
          {activeTab === "legal" && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-12">
              <div className="space-y-2">
                ...
                <h1 className="text-4xl md:text-5xl font-bold text-white tracking-tight">Legal Notice & Terms</h1>
